# Kubernetes on OTC with Terraform
...
This will install a Kubernetes cluster on an Opentelekomcloud Cloud.

## Approach
The terraform configuration inspects variables found in
[variables.tf](variables.tf) to create resources in your OTC cluster.
...
## Terraform
Terraform will be used to provision all of the OTC resources. It is also
...
Ensure your OTC **Identity v2** credentials are loaded in environment
variables. This can be done by downloading a credentials .rc file from your
OTC dashboard and sourcing it:

```
$ source ~/.stackrc
```

Ensure that you have your OTC credentials loaded into Terraform
...
|---------|-------------|
|`cluster_name` | All OTC resources will use the Terraform variable`cluster_name` (default`example`) in their name to make it easier to track. For example the first compute resource will be named`example-kubernetes-1`. |
|`network_name` | The name to be given to the internal network that will be generated |
|`dns_nameservers`| An array of DNS name server names to be used by hosts in the internal subnet. | 
|`floatingip_pool` | Name of the pool from which floating IPs will be allocated |
|`external_net` | UUID of the external network that will be routed to |
|`flavor_k8s_master`,`flavor_k8s_node`,`flavor_etcd`, `flavor_bastion`,`flavor_gfs_node` | Flavor depends on your OTC installation, you can get available flavor IDs through`nova flavor-list` |
...
```bash
$ terraform init contrib/terraform/otc
```
...
```bash
$ terraform apply -state=contrib/terraform/otc/terraform.tfstate -var-file=my-terraform-vars.tfvars contrib/terraform/otc
```

if you chose to create a bastion host, this script will create
`contrib/terraform/otc/k8s-cluster.yml` with an ssh command for ansible to
...
issued from the project's root directory
```bash
$ terraform destroy -state=contrib/terraform/otc/terraform.tfstate -var-file=my-terraform-vars.tfvars contrib/terraform/otc
```
...
```
$ ansible -i contrib/terraform/otc/hosts -m ping all
...
> calico requires allowing service's and pod's subnets on according OTC Neutron ports
...
For calico configure OTC Neutron ports: [OTC](/docs/OTC.md)

## Deploy kubernetes:

```
$ ansible-playbook --become -i contrib/terraform/otc/hosts cluster.yml
```
...
```bash
$ ansible-playbook --become -i contrib/terraform/otc/hosts ./contrib/network-storage/glusterfs/glusterfs.yml
```
